...
public final class LoremIpsum {
    ...
    public static let sharedInstance = LoremIpsum()
    ...
    private init() {
        ...
        let range = loremIpsum.startIndex..<loremIpsum.endIndex
        let characters = NSCharacterSet.whitespaceAndNewlineCharacterSet()
        loremIpsum.enumerateSubstringsInRange(range, options: [.BySentences]) { substring, _, _, _ in
    ...
    public func generateBySentences(count: Int, fromStart: Bool) -> String {
        let start = fromStart ? 0 : random() % sentences.count - count
        let end = start.advancedBy(count)
        return sentences[start..<end].map { $0.string }.joinWithSeparator(" ")
    }

    public func generateByWords(var count: Int) -> String {
        var index: Int
        repeat {
            index = random() % sentences.count
        } while sentences[index].wordCount < count

        let string = sentences[index].string
        var range = string.startIndex..<string.endIndex
        sentences[index].string.enumerateSubstringsInRange(range, options: [.ByWords]) { _, subrange, _, stop in
